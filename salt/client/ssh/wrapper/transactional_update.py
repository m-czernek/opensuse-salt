...
def _ssh_state(chunks, st_kwargs, kwargs, test=False):
    ...
    cmd = "transactional_update.pkg {}/salt_state.tgz test={} pkg_sum={} hash_type={}".format(
...
def low(data, **kwargs):
    ...
    with salt.client.ssh.state.SSHHighState(
        ...
        trans_tar_sum = salt.utils.hashutils.get_hash(trans_tar, __opts__["hash_type"])
        cmd = "transactional_update.pkg {}/salt_state.tgz pkg_sum={} hash_type={}".format(
...
def single(fun, name, test=None, **kwargs):
    ...
    # We use transactional_update.pkg to execute the "state package"
    cmd = "transactional_update.pkg {}/salt_state.tgz test={} pkg_sum={} hash_type={}".format(
        opts["thin_dir"], test, trans_tar_sum, opts["hash_type"]
    )
    ...
    # Run the transactional_update.pkg command on the target
    stdout, stderr, _ = single.cmd_block()
